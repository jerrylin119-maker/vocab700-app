"""
Vocabulary Generator for 700 Core English Words Dataset.
Creates default_vocab.json and vocab_template.csv.
"""
...
# Curated high-frequency vocabulary entries spanning 70 Units (700 words total)
# Each entry: (word, phonetic, pos, english_def, chinese_def, example_sentence)
BASE_VOCAB = [
    # Unit 1 (1-10)
...
WORD_POOLS = [
    ...
    [
        ...
        ("brief", "/briːf/", "adj.", "Of short duration; concise in expression", "簡短的；短載的", "The director gave a brief introduction before the presentation."),
...
def load_extended_vocabulary():
    """Generates the full 700-word dataset."""
    words = list(BASE_VOCAB)
    for p in WORD_POOLS:
        words.extend(p)

    # Systematic rich vocabulary database
    extra_sets = [
        # Unit 11-20
    ...
    words.extend(extra_sets)
    return words

def generate_700_word_dataset():
    """Builds a verified list of 700 structured words."""
    base = load_extended_vocabulary()
    seen = {w[0].lower() for w in base}
    full_list = list(base)

    # Curated word banks to reach 700 words
    word_sources = [
        # Lexicon Block A (100 words)
        ("criterion", "/kraɪˈtɪəriən/", "n.", "A principle or standard by which something may be judged or decided", "準則；標準", "Academic achievement is not the sole criterion for admission."),
        ("critical", "/ˈkrɪtɪkl/", "adj.", "Expressing adverse or disapproving comments; of greatest importance", "關鍵的；批判的", "Timely decision-making is critical to emergency medical response."),
        ("criticize", "/ˈkrɪtɪsaɪz/", "v.", "To indicate the faults of someone or something in a disapproving way", "批評；指責", "It is easy to criticize, but much harder to propose constructive solutions."),
        ("crucial", "/ˈkruːʃl/", "adj.", "Decisive or critical, especially in the success or failure of something", "至關重要的", "Water supply is crucial for agricultural sustainability."),
        ...
        ("custom", "/ˈkʌstəm/", "n.", "A widely accepted way of behaving or doing something specific to a society", "習俗；慣例", "It is a local custom to exchange gifts during the new year festival."),
        ("damage", "/ˈdæmɪdʒ/", "n.", "Physical harm caused to something in such a way as to impair its value", "損害；傷害", "The hurricane caused severe damage to coastal infrastructure."),
        ...
        ("decorate", "/ˈdekəreɪt/", "v.", "To make something look more attractive by adding ornament or color", "裝飾；佈置", "They spent the weekend decorating the living room for Christmas."),
        ("decrease", "/dɪˈkriːs/", "v.", "To make or become smaller or fewer in size, amount, intensity, or degree", "減少；降低", "Traffic noise decreased noticeably after midnight."),
        ("dedicate", "/ˈdedɪkeɪt/", "v.", "To devote time, effort, or oneself to a particular task or purpose", "奉獻；致力於", "She decided to dedicate her career to cancer research."),
        ...
        ("define", "/dɪˈfaɪn/", "v.", "To state or describe exactly the nature, scope, or meaning of", "定義；界定", "Dictionaries define words using clear, precise language."),
        ...
        ("delicate", "/ˈdelɪkət/", "adj.", "Very fine in texture or structure; easily broken", "精緻的；脆弱的", "The antique glass vase is extremely delicate and requires careful handling."),
        ("delight", "/dɪˈlaɪt/", "n.", "Great pleasure or joy", "欣喜；愉快", "The children squealed with delight upon seeing the magical performance."),
        ("deliver", "/dɪˈlɪvər/", "v.", "To bring and hand over a parcel or goods to the proper recipient", "遞送；發表", "Couriers deliver thousands of packages across the island daily."),
        ("demand", "/dɪˈmɑːnd/", "n.", "An insistent and peremptory request, made as if by right", "要求；需求", "Consumer demand for electric vehicles has surged recently."),
        ("demonstrate", "/ˈdemənstreɪt/", "v.", "To clearly show the existence or truth of something by giving proof or evidence", "示範；證明", "The professor demonstrated the experiment in the chemistry lab."),
        ("dense", "/dens/", "adj.", "Closely compacted in substance; crowded", "稠密的；濃密的", "A dense fog settled over the harbour early this morning."),
        ("deny", "/dɪˈnaɪ/", "v.", "To state that one refuses to admit the truth or existence of", "否認；拒絕", "The suspect continued to deny any involvement in the incident."),
        ...
        ("deposit", "/dɪˈpɒzɪt/", "v.", "To put or keep in a safe place", "存款；放置", "She went to the bank to deposit her monthly salary."),
        ("depress", "/dɪˈpres/", "v.", "To make someone feel utterly dispirited or dejected", "使沮喪；使消沉", "Gloomy rainy weather often tends to depress his spirits."),
        ...
        ("deserve", "/dɪˈzɜːv/", "v.", "To be worthy of or qualified for a reward or punishment", "值得；應得", "Hardworking students deserve recognition for their academic efforts."),
        ("design", "/dɪˈzaɪn/", "v.", "To decide upon the look and functioning of a building or garment", "設計；構想", "Architects design sustainable homes that utilize natural sunlight."),
        ...
        ("destroy", "/dɪˈstrɔɪ/", "v.", "To end the existence of something by damaging or attacking it", "破壞；摧毀", "The forest fire destroyed hundreds of acres of natural habitat."),
        ...
        ("determine", "/dɪˈtɜːmɪn/", "v.", "To cause something to occur in a particular way; decide", "決定；下定決心", "Your dedication will determine how far you progress."),

        ("develop", "/dɪˈveləp/", "v.", "To grow or cause to grow and become more mature or advanced", "發展；開發", "Engineers develop innovative mobile applications for users worldwide."),
        ...
        ("devote", "/dɪˈvəʊt/", "v.", "To give all or a large part of one's time or resources to a person or activity", "奉獻；致力", "He decided to devote his life to humanitarian work."),
        ("differ", "/ˈdɪfər/", "v.", "To be unlike or dissimilar", "相異；不同", "Opinions differ widely regarding the new taxation policy."),
        ("dimension", "/daɪˈmenʃn/", "n.", "A measurable extent of some kind, such as length or breadth", "維度；尺寸", "Computer graphics add a realistic three-dimensional dimension to movies."),
        ("diminish", "/dɪˈmɪnɪʃ/", "v.", "To make or become less", "減少；縮減", "Pain began to diminish after taking the prescribed medicine."),
        ("direct", "/dəˈrekt/", "adj.", "Extending or moving from one place to another by the shortest way without changing direction", "直接的；指導", "We took a direct flight from Taipei to San Francisco."),
    ...
    for w in word_sources:
        if w[0].lower() not in seen:
            seen.add(w[0].lower())
            full_list.append(w)
